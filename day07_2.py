...
def solve(input):
    ...
    for line in input.split('\n'):
        ...
        commandChildren = []
        first = commandSplit[0]
        if (len(commandSplit) > 1):
            second = commandSplit[1]
            commandChildren = [child for child in second.split(", ")]
            children.extend(commandChildren)
        commandName = first.split(" ")[0]
        commandWeight = int(first.split(" ")[1][1:-1])
        commands[commandName] = { "weight": commandWeight, "children": commandChildren }

    # second pass - find root
    for key, value in commands.iteritems():
        if key not in children and len(value["children"]):
            root = key

    calculateWeightOfEntireSubTower(root, commands)

def calculateWeightOfEntireSubTower(root, commands):
    subTowerWeight = 0
    children = commands[root]["children"]
    if len(children) == 0:
        subTowerWeight += commands[root]["weight"]
    else:
        childWeights = []
        global firstUnbalancedChild
        for child in children:
            childWeight = calculateWeightOfEntireSubTower(child, commands)
            childWeights.append(childWeight)

        if len(set(childWeights)) > 1:
            if not firstUnbalancedChild:
                firstUnbalancedChild = children[0]
                setWeights = list(set(childWeights))
                print commands[firstUnbalancedChild]["weight"] - (setWeights[0] - setWeights[1])
        subTowerWeight += commands[root]["weight"]
        subTowerWeight += sum(childWeights)

    return subTowerWeight
