...
def hasOriginInZero(pipes, items):
    global connectingItems
    for item in items:
        if item not in connectingItems:
            connectingItems.append(item)
            hasOriginInZero(pipes, pipes[item])

def appearsInAnyGroup(item):
...
def solve(input):
    ...
    for line in input.split("\n"):
        ...
        destinations = [int(x) for x in line.strip().split(" <-> ")[1].split(", ")]
        pipes[source] = destinations

    global distinctGroups
    global connectingItems

    for i, val in pipes.iteritems():
        connectingItems = []
        connectingItems.append(i)
        hasOriginInZero(pipes, pipes[i])
        if not appearsInAnyGroup(i):
            distinctGroups.append(set(connectingItems))

    print len(distinctGroups)
