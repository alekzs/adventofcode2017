...
def solve(input):
    ...
    for line in input.split('\n'):
        ...
        destination = commandSplit[0]
        operation = commandSplit[1]
        value = int(commandSplit[2])
        conditionRegister = commandSplit[4]
        condition = commandSplit[5]
        conditionValue = int(commandSplit[6])
        ...
        if (condition == ">" and registers[conditionRegister] > conditionValue) or \
            (condition == "<" and registers[conditionRegister] < conditionValue) or \
            (condition == "<=" and registers[conditionRegister] <= conditionValue) or \
            (condition == "!=" and registers[conditionRegister] != conditionValue) or \
            (condition == ">=" and registers[conditionRegister] >= conditionValue) or \
            (condition == "==" and registers[conditionRegister] == conditionValue):
            isValidCommand = True

        if isValidCommand:
            registers[destination] += value * (1 if operation == "inc" else -1)

        highestEver = max(highestEver, registers[destination])
